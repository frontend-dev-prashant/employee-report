// File: src/components/EmployeeTableNew.jsx
// Purpose: Employee CRUD + login (OTP) + soft-delete.
// - Inserts set created_at & last_login to same value (client-managed).
// - OTP verify sets login_status = 'logged_in' and updates last_login.
// - Deleting sets login_status = 'logged_out' and deleted_at/deleted_by.
...
/* -------------------------
   Helper: centralized time management
   ------------------------- */
const getNowIso = () => new Date().toISOString();

/* -------------------------
   Small helper: escape for HTML injection safety in SweetAlert
   ------------------------- */
const escapeHtml = (s) =>
    String(s ?? "")
        .replace(/&/g, "&amp;")
...
export default function EmployeeTableNew({ loginButton }) {
    ...
    // Fetch rows (exclude soft-deleted)
    const fetchRows = async (showAlert = false) => {
        setLoading(true);

        // destroy datatable instance (if exists) before React updates DOM
        if (dtRef.current) {
    ...
    // CREATE: when inserting, set created_at and last_login to same timestamp
    const createRow = async (payload) => {
        const nowIso = getNowIso();

        // ensure created_at and last_login are identical
        const insertPayload = {
            ...payload,
            created_at: nowIso,
            last_login: nowIso,
            // default initial login status is logged_in; recruiter asked to set last_login to created_at
    ...
    // SOFT DELETE: set deleted_at, deleted_by and set login_status to logged_out
    ...
    /* ======================
       UI handlers: Add / Edit / Delete / View
       ====================== */
    ...
    `,
        ...
        if (formValues.name.length > 50) {
            Swal.fire("Validation", "Name must be 50 characters or less.", "warning");
            return;
        }
        if (formValues.email.length > 50) {
            Swal.fire("Validation", "Email must be 50 characters or less.", "warning");
            return;
        }
    ...
    const handleDelete = async (row) => {
        const { value: deletedBy } = await Swal.fire({
            title: `Delete (${escapeHtml(row.emp_code)}-${escapeHtml(row.name)})?`,
    ...
    /* ======================
       Login flow: email -> OTP -> verify
       - When OTP verified: set login_status='logged_in' and update last_login to now.
       - Email is required and validated before sending OTP.
       ====================== */
    const handleLoginClick = async () => {
        // 1) Ask for email (required & validated)
        ...
        `,
            ...
            preConfirm: () => {
                ...
                if (v.length > 50) {
                    Swal.showValidationMessage("Email must be 50 characters or less");
                    return null;
                }
                return v;
            },
        ...
        if (!email) return; // canceled or validation failed

        // 2) Check email in supabase (exclude deleted)
        let found;
        try {
            const { data, error } = await supabase
                .from("employees")
            ...
            Swal.fire({ icon: "error", title: "Error", text: "Failed to check email. See console." });
            return;
        }
        ...
        // 3) Show OTP input UI and validate via preConfirm (safer than reading DOM later)
        const { value: otpValue } = await MySwal.fire({
            title: "Enter OTP",
            html: `
            <p class="mb-2 text-gray-700">OTP sent to <b>${escapeHtml(email)}</b></p>
        ...
        `,
            ...
            preConfirm: () => {
                ...
                if (!otp || otp.length !== 4) {
                    Swal.showValidationMessage("Please enter the 4-digit OTP");
                    return null;
                }
                return otp;
            },
        ...
        if (!otpValue) return; // canceled or validation failed
        ...
        // 4) Update login_status and last_login to now
        try {
            const nowIso = getNowIso();
            const { data: updated, error: updErr } = await supabase
                .from("employees")
                .update({ login_status: "logged_in", last_login: nowIso })
                .eq("id", user.id)
                .select("*");

            if (updErr) {
                Swal.fire("Error", updErr.message || "Update failed", "error");
                return;
            }
            ...
            Swal.fire("Error", "Failed to update login status.", "error");
        }
    };

    /* ======================
       OTP helper: auto-move and auto-submit
       ====================== */
        ...
        inputs.forEach((inp, idx) => {
            inp.addEventListener("input", () => {
                ...
                if ([...inputs].every((i) => i.value)) {
                    // click confirm (will trigger preConfirm)
                    Swal.clickConfirm();
                }
            });
    ...
    return (
        <>
            ...
            <nav className="bg-white border border-gray-200 dark:border-gray-700 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-800 shadow-sm">
                <div className="container flex flex-wrap justify-between items-center mx-auto">
                    ...
                    <Link to="/" className="flex items-center" title="Home">
                        ...
                        <span className="ml-2 font-semibold">Employee Report</span>
                    </Link>
                    ...
                    <div className={`w-full md:block md:w-auto ${isOpen ? "block" : "hidden"}`} id="mobile-menu">
                        <ul className="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
                            ...
                            {/* Conditional Login button (only on employees page) */}
                            {location.pathname === "/employees-new" && (
                                <li
                                    onClick={handleLoginClick}
                                    className="block py-2 cursor-pointer pr-4 pl-3 text-gray-700 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent"
                                >
                                    Login
                                </li>
                            )}
            ...
            < div className="bg-gray-100 p-2 items-center" >
                <div className="max-w-full w-full mx-auto">
                    <div className="shadow">
                        {loading && rows.length === 0 ? (
                            ...
                            <>
                                <div className="flex justify-end text-right gap-2 mb-2">
                                    <div>
                                        <button
                                            title="Add Employee"
                                            onClick={handleAdd}
                                    ...
                                    <div>
                                        <button
                                            ...
                                            onClick={async () => {
                                                ...
                                                // If data fetched successfully, reinitialize the data table
                                                if (res.ok) {
                                                    // Optionally: Clear existing table data
                                                    clearDataTable();  // if this function exists
                                ...
                                <div className="overflow-y-auto h-[86vh]">
                                    ...
                                    >
                                        <thead className="bg-gradient-to-r from-blue-500 to-teal-500 text-white sticky top-1">
                                            ...
                                            {rows.map((r, i) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="p-4 text-center space-x-3">
                                                        <button onClick={() => handleView(r)} className="p-2 text-indigo-600 rounded-md hover:bg-indigo-200 transition duration-300">
                                                            <EyeIcon className="h-5 w-5" />
                                                        </button>
                                                        <button onClick={() => handleEdit(r)} className="p-2 text-yellow-600 rounded-md hover:bg-yellow-200 transition duration-300">
                                                            <PencilSquareIcon className="h-5 w-5" />
                                                        </button>
                                                        <button onClick={() => handleDelete(r)} className="p-2 text-red-600 rounded-md hover:bg-red-200 transition duration-300">
